            ...
            request()
                ...
                .then((res) => {
                    ...
                    chai.expect(res.body).to.containSubset(
                        JSON.parse(`[{"name":"faîtage","definition":"Le haut de la charpente","uuid":"ad1afbd2-77d2-404e-bbca-3f352f7f09a2"},{"name":"mur de refend","definition":"Mur porteur interieur","uuid":"c067b934-d98f-4d0a-a4a3-c89c05a00e4a"}]`)
                    )
                    done()
                }).catch(done)
        })
        ...
        it('it should be return the element by id', (done) => {
            request()
                ...
                .then((res) => {
                    chai.expect(res).to.have.status(200);
                    chai.expect(res.body).to.containSubset(
                        JSON.parse(`{"name":"faîtage","definition":"Le haut de la charpente","uuid":"ad1afbd2-77d2-404e-bbca-3f352f7f09a2"}`)
                    )
                    done()
                }).catch(done)
        })
    ...
    describe('Post', () => {
        ...
        it('it should return 422 when name is null', (done) => {
            request()
                ...
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"name":null,"definition":"Yolo"}')
                .then((res) => {
                    chai.expect(res).to.have.status(422);
                    done()
                }).catch(done)
        ...
        it('it should return 422 when name is empty', (done) => {
            request()
                ...
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"name":"","definition":"Yolo"}')
                .then((res) => {
                    chai.expect(res).to.have.status(422);
                    done()
                }).catch(done)
        })
        it('it should return 422 when definition is not defined', (done) => {
            request()
                .post(`/${subject}/`)
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
        ...
        it('it should return 422 when definition is null', (done) => {
            request()
                .post(`/${subject}/`)
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"definition":null,"name":"Yolo"}')
                .then((res) => {
                    chai.expect(res).to.have.status(422);
                    done()
                }).catch(done)
        })
        it('it should return 422 when definition is empty', (done) => {
            request()
                .post(`/${subject}/`)
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"definition":"","name":"Yolo"}')
                .then((res) => {
                    chai.expect(res).to.have.status(422);
                    done()
                }).catch(done)
        ...
        it(`it should return 201 when create a valid ${subject}`, (done) => {
            request()
                ...
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"name":"Yolo","definition":"Yolo"}')
                .then((res) => {
                    chai.expect(res).to.have.status(201);
                    chai.expect(res.body).to.containSubset(
                        JSON.parse(`{"name":"Yolo","definition":"Yolo"}`)
                    )
    ...
    describe('Put', () => {
        ...
        it('it should return 422 when definition is null', (done) => {
            request()
                ...
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"name":"Yolo","definition":null}')
                .then((res) => {
                    chai.expect(res).to.have.status(422);
                    done()
                }).catch(done)
