            ...
            request()
                ...
                .then((res) => {
                    ...
                    chai.expect(res.body).to.containSubset(
                        JSON.parse(`[{"name":"Headphone sony","end":"23/12/2022","store":"Boulanger","uuid":"ad1afbd2-77d2-404e-bbca-3f352f7f09a2"},{"name":"lightsaber","end":"15/06/2022","store":"Jedi Temple","uuid":"c067b934-d98f-4d0a-a4a3-c89c05a00e4a"}]`)
                    )
                    done()
                }).catch(done)
        })
        ...
        it('it should be return the element by id', (done) => {
            request()
                ...
                .then((res) => {
                    chai.expect(res).to.have.status(200);
                    chai.expect(res.body).to.containSubset(
                        JSON.parse(`{"name":"Headphone sony","end":"23/12/2022","store":"Boulanger","uuid":"ad1afbd2-77d2-404e-bbca-3f352f7f09a2"}`)
                    )
                    done()
                }).catch(done)
        })
    ...
    describe('Post', () => {
        ...
        it('it should return 422 when name is null', (done) => {
            request()
                ...
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"name":null,"end":"Yolo","store":"Yolo"}')
                .then((res) => {
                    chai.expect(res).to.have.status(422);
                    done()
                }).catch(done)
        ...
        it('it should return 422 when name is empty', (done) => {
            request()
                ...
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"name":"","end":"Yolo","store":"Yolo"}')
                .then((res) => {
                    chai.expect(res).to.have.status(422);
                    done()
                }).catch(done)
        })
        it('it should return 422 when end is not defined', (done) => {
            request()
                .post(`/${subject}/`)
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
        ...
        it('it should return 422 when end is null', (done) => {
            request()
                .post(`/${subject}/`)
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"end":null,"name":"Yolo","store":"Yolo"}')
                .then((res) => {
                    chai.expect(res).to.have.status(422);
                    done()
                }).catch(done)
        })
        it('it should return 422 when end is empty', (done) => {
            request()
                .post(`/${subject}/`)
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"end":"","name":"Yolo","store":"Yolo"}')
                .then((res) => {
                    chai.expect(res).to.have.status(422);
                    done()
                }).catch(done)
        ...
        it('it should return 422 when store is null', (done) => {
            request()
                ...
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"store":null,"name":"Yolo","end":"Yolo"}')
                .then((res) => {
                    chai.expect(res).to.have.status(422);
                    done()
                }).catch(done)
        ...
        it(`it should return 201 when create a valid ${subject}`, (done) => {
            request()
                ...
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"name":"Yolo","end":"Yolo","store":"Yolo"}')
                .then((res) => {
                    chai.expect(res).to.have.status(201);
                    chai.expect(res.body).to.containSubset(
                        JSON.parse(`{"name":"Yolo","end":"Yolo","store":"Yolo"}`)
                    )
    ...
    describe('Put', () => {
        ...
        it('it should return 422 when end is null', (done) => {
            request()
                ...
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"name":"Yolo","end":null,"store":"Yolo"}')
                .then((res) => {
                    chai.expect(res).to.have.status(422);
                    done()
                }).catch(done)
        ...
        it('it should return 422 when store is null', (done) => {
            request()
                ...
                .set('Authorization', `Bearer ${jwt_user1}`)
                .set('Content-type', 'application/json')
                .send('{"name":"Yolo","end":"Yolo","store":null}')
                .then((res) => {
                    chai.expect(res).to.have.status(422);
                    done()
                }).catch(done)
